...
VkCore::RenderPass::RenderPass(const Context &context,
    ...
    for (uint32_t index = 0; index < attachments.size(); ++index) {
        attachmentDescriptors.push_back(
                VkAttachmentDescription{
                        .format = attachments[index]->vkFormat(),
                        .samples = attachments[index]->VkSampleCount(),
                        .loadOp = loadOp[index],
                        .storeOp = storeOp[index],
                        .stencilLoadOp = attachments[index]->isStencil() ? loadOp[index]
                                                                         : VK_ATTACHMENT_LOAD_OP_DONT_CARE,
                        .stencilStoreOp = attachments[index]->isStencil() ? storeOp[index]
...
VkCore::RenderPass::RenderPass(const VkCore::Context &context, const std::vector<VkFormat> &formats,
                               ...
                               bool multiview, const std::string &name) : device_{
        context.device()} {
