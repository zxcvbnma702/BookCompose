...
package com.tal.xes.bookcompose.onnx.ai.tokenizer
...
import com.tal.xes.bookcompose.onnx.LocalDiffusionContract.KEY_INPUT_IDS
import com.tal.xes.bookcompose.onnx.ModelLoader
import com.tal.xes.bookcompose.onnx.OrtEnvironmentManager
import com.tal.xes.bookcompose.onnx.extensions.halfCorner
import com.tal.xes.bookcompose.onnx.extensions.toArrays
...
internal class EnglishTextTokenizer(private val context: Context, private val basePath: String) : LocalDiffusionTextTokenizer {
    ...
    override fun initialize() {
        ...
        // Here we load the ONNX text encoder model from downloaded path
        val modelPath = File(basePath, LocalDiffusionContract.TOKENIZER_MODEL).absolutePath
    ...
    override fun tensor(ids: IntArray?): OnnxTensor? {
        ...
        result.close()
        val tensor = OnnxTensor.createTensor(OrtEnvironmentManager.environment, lastHiddenState)
        return tensor
    }
    ...
    private fun bpe(token: String): List<String> {
        ...
        while (true) {
            ...
            var minValue = 0
            for (pair in pairs) {
                if (!bpeRanks.containsKey(pair)) continue
    ...
    private fun loadEncoder(): Map<String, Int> {
        ...
        try {
            val file = File(basePath, LocalDiffusionContract.TOKENIZER_VOCABULARY)
            if (!file.exists()) {
                Log.e("EnglishTextTokenizer", "Vocabulary file not found: ${file.absolutePath}")
                return map
            }
            ...
            Log.e("EnglishTextTokenizer", "Failed to load encoder", e)
        }
        return map
    }
    ...
    private fun loadBpeRanks(): Map<Pair<String, String>, Int?> {
        ...
        try {
            val file = File(basePath, LocalDiffusionContract.TOKENIZER_MERGES)
            if (!file.exists()) {
                Log.e("EnglishTextTokenizer", "Merges file not found: ${file.absolutePath}")
                return result
            }
            val stream = FileInputStream(file)
            ...
            while (reader.readLine().also { ln -> line = ln ?: "" } != null) {
                ...
                if (array.size >= 2) {
                    result[Pair(array[0], array[1])] = count++
                }
            }
            reader.close()
        } catch (e: Exception) {
            Log.e("EnglishTextTokenizer", "Failed to load BPE ranks", e)
        }
        return result
    }
